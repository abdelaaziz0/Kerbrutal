...
// Dialer interface for custom network dialing (e.g. SOCKS5)
...
// WithDialer used to configure client with a custom dialer for network connections.
func WithDialer(d Dialer) func(*Settings) {
	return func(s *Settings) {
		s.dialer = d
	}
}

// Dialer returns the client's custom dialer, if any.
func (s *Settings) Dialer() Dialer {
	return s.dialer
}
