...
func (cl *Client) sendToKDC(b []byte, realm string) ([]byte, error) {
	...
	if cl.settings != nil && cl.settings.Dialer() != nil {
		// Force TCP when using a proxy
		rb, errtcp := cl.sendKDCTCP(realm, b)
		if errtcp != nil {
			if e, ok := errtcp.(messages.KRBError); ok {
				return rb, e
			}
			return rb, fmt.Errorf("communication error with KDC via proxied TCP: %v", errtcp)
		}
		return rb, nil
	}
...
func (cl *Client) dialSendTCP(kdcs map[int]string, b []byte) ([]byte, error) {
	...
	for i := 1; i <= len(kdcs); i++ {
		...
		if cl.settings != nil && cl.settings.Dialer() != nil {
			...
			if err != nil {
				errs = append(errs, fmt.Sprintf("error dialing %s via proxy: %v", kdcs[i], err))
				continue
			}
		} else {
		...
		// conn is guaranteed to be a net.Conn
